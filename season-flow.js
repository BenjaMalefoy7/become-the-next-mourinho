const BTM_SEASON_FLOW_VERSION = "0.28B";
(function () {
  ...
  function nextInfo(career) {
    ...
    return {
      match,
      date: targetDate,
      days: diff(career.currentDate, targetDate),
      label: typeof getMatchLabel === "function" ? getMatchLabel(match) : match.homeClubName + " vs " + match.awayClubName
    };
  }
  ...
  function recover(career) {
    (Array.isArray(career.players) ? career.players : []).forEach((player) => {
      player.condition = Math.min(100, (Number(player.condition ?? 100) || 100) + 2);
    });
  }
  ...
  function saveSimulatedMatchV028B() {
    const active = activeIndex();
    ...
    return {
      ok: true,
      career,
      result: career.lastMatchResult || userResult.result,
      matchday: userMatch.matchday,
      simulatedMatches: 1 + otherCount,
      message: "Journée simulée."
    };
  }
  ...
  window.saveSimulatedMatch = saveSimulatedMatchV028B;
  ...
  function render() {
    ...
    box.innerHTML = `
      <div class="season-v014-left">
        <div class="season-v014-date"><span>Date du jour</span><strong>${e(fr(career.currentDate))}</strong></div>
        <div class="season-v014-next">
          <span>Prochain match</span><strong>${e(next.label)}</strong>
          <small>${next.date ? ` · ${e(fr(next.date))} · ${due ? "aujourd’hui" : "J-" + Math.max(0, next.days)}` : ""}</small>
          <div class="season-v014-warning">${e(due ? "Match à jouer aujourd’hui" : message)}</div>
        </div>
      </div>
      <div class="season-v014-actions">
        <button class="secondary-btn" id="season-next-day" ${due ? "disabled" : ""}>${due ? "Match à jouer" : "Passer au jour suivant"}</button>
        <button class="primary-btn" id="season-next-match">${due ? "Aller au match" : "Prochain match plus tard"}</button>
        <button class="secondary-btn" id="season-open-mail">Courrier ${unread ? `<span class='season-v014-mail-pill'>${unread}</span>` : ""}</button>
      </div>`;
  ...
  function updateCopy() {
    if (typeof setText === "function") setText("dashboard-description", "V0.28B : la simulation passe par match-engine.js et league-sim.js, sans dépendre des anciens saveSimulatedMatch empilés.");
    const footer = document.querySelector(".sidebar-footer");
    if (footer) footer.textContent = "V0.28B — Simulation pure";
  }

  const previousRefresh = typeof refreshUI === "function" ? refreshUI : null;
  refreshUI = function refreshUIV028BSeasonFlow() {
